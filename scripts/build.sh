...
if [ -d partials ]; then
  ...
  # /_partials/* indexing is blocked via public/_headers (X-Robots-Tag) —
  # a robots.txt outside the domain root has no effect on crawlers.
  rm -f public/_partials/robots.txt
